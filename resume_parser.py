...
def getParsedData():
    ...
    file_name = file.filename.split('.')[0] + datetime.now().strftime("%Y%m%d%H%M%S") + '.' + file.filename.split('.')[-1]
    ...
    if file_name.endswith('.doc'):
        ...
        links.append(re.findall('www.+[.]com',' '.join(document)))
    ...
    if len(experiences)>0 and len(experiences[0])>2:
        for experience in experiences:
            if 'PROJECT' not in experience[0].upper() or len(experiences)==1:
                position,company,date = extractDataJob(experience[1:],finder,tagger)
...
def open_docx_file(file_name,nlp):
    ...
    for block in iter_block_items(document):
        if isinstance(block, Paragraph):
            ...
            while start_ind<len(block.rows):
                cleaned = [*set([cell.text for cell in block.rows[start_ind].cells if cell.text.strip()!='' and len(cell.text.strip())>2])]
                if len(cleaned)>1:
                    break
                else:
            ...
            for i, row in enumerate(block.rows[start_ind+1:]):
                cleaned = [*set([cell.text for cell in row.cells if cell.text.strip()!='' and len(cell.text.strip())>2])]
            ...
            temp_text = [cell.text.strip().upper() for cell in block.rows[start_ind].cells[1:]] if len(block.rows)>start_ind and len(block.rows[start_ind].cells)>1 else []
            ...
            if horizontal and table:
                for i, row in enumerate(block.rows):
                    ...
                    cleaned_text = []
                    [cleaned_text.append(x) for x in temp_text if x not in cleaned_text and x.strip()!='']
                    
                    if len(cleaned_text)>1:
                        row_data = [str(cleaned_text[0]) + ':' + str(item) for item in cleaned_text[1:] if len(item.strip())>2]
                        text = text + row_data
                ...
                for i, row in enumerate(block.rows):
                    ...
                    if len(cleaned_text)>1:
                        row_data = [str(key) + ':' + str(item) for (key,item) in list(zip(keys, list(temp_text))) if len(item.strip())>2]
                        text = text + row_data
                    elif len(cleaned_text)>0:
                        text.append(cleaned_text[0]) 
    ...
    text = [remove_non_ascii(line.replace('\t', ' ')) for line in text if line != '' and len(remove_non_ascii(line).strip())>1]
    
    doc = docx2python(file_name)
    header_text = [i for i in list(iter_paragraphs(doc.header)) if i.replace('\t','').strip()!='' and not '.png' in i and not '.jpg' in i and not '.gif' in i and not '.jpeg' in i and not '.Tiff' in i]
    if header_text != []:
        text = header_text + text
     
    if len(text)>0:
      doc2 = nlp(text[0].title())
      names = [ent.text for ent in doc2.ents if ent.label_ == 'PERSON' or ent.label_ == 'GPE']
...
def open_pdf(file,nlp):
    ...
    tables = False
    labels = ['TECHNOLOGY','DESIGNATION','ROLE','CLIENT','ORGANIZATION','PROJECT','TITLE','CUSTOMER','CLIENT DOMAIN','STAKEHOLDERS','ENVIRONMENT','SKILLS','APPLICATIONS','DOMAIN','INDUSTRY','SERVICE','DESCRIPTION','RESPONSIBILITES','SIZE','PERIOD','DURATION','TECHNOLOGY','TOOLS','ACHIEVEMENTS','PROGRAMMING LANGUAGES','DATABASES','SKILLS','PROTOCOLS','MULESOFT','PROJECTS','TEAM SIZE','FRAMEWORKS','LOCATION','ROLE/ACTIVITIES','NAME','ROLES','PROJECT NAME','SCORE','QUALIFICATION','SCHOOL/COLLEGE','SCHOOL/COLLEGE YEAR','INSTITUTION','DEGREE /CERTIFICATE','SUBJECT']
            ...
            for page in pdf.pages:
                ...
                for cluster in pdfplumber.utils.cluster_objects(
                    ...
                    if 'text' in cluster[0]:
                        temp.append(' '.join([i['text'] for i in cluster if 'text' in i]))
                        ...
                        horizontal = next((any(label == x[0].strip().upper() for label in labels) for x in cluster[0]['table'][1:] if len(x)>0 and x!=None and x[0]!=None and any(label == x[0].strip().upper() for label in labels)), False)
                        row_data = [x.strip().upper() for x in cluster[0]['table'][0][1:] if x!=None]
                        ...
                        if horizontal and table:
                            for i in range(0,len(cluster[0]['table'])):
                                if len(cluster[0]['table'][i])>1:
                                    row_data = [str(cluster[0]['table'][i][0]) + ':' + str(x) for x in cluster[0]['table'][i][1:] if x != None and len(x.strip())>2]
                                    temp = temp + row_data
                        elif table:
                            keys = cluster[0]['table'][0]
                            for i in range(1,len(cluster[0]['table'])):
                                row_data = [str(key) + ':' + str(item) for (key,item) in list(zip(keys, list(cluster[0]['table'][i]))) if item != None and len(item.strip())>2]
                                temp = temp + row_data
                        else:
                            for i, row in enumerate(cluster[0]['table']):
                ...
                if start == .5:
                    ...
                    if start == .5:
                        ...
                        if ind == 0:
                            ...
                            doc2 = nlp(stripped_doc[0].title()) if len(stripped_doc)>0 else []
                            names = [ent.text for ent in doc2.ents if ent.label_ == 'PERSON' or ent.label_ == 'GPE'] if doc2!=[] else []
                    else:
                        ...
                        if ind == 0:
                            for doc in [top.extract_text(x_tolerance=1).split('\n'),  left.extract_text(x_tolerance=1).split('\n'), right.extract_text(x_tolerance=1).split('\n')]:
                                stripped_doc = [i for i in doc if i!=None and i.strip()!='' and len(i.strip())>1]
                                doc2 = nlp(stripped_doc[0].title()) if len(stripped_doc)>0 else []
                                names = [ent.text for ent in doc2.ents if ent.label_ == 'PERSON' or ent.label_ == 'GPE'] if doc2!=[] else [] 
                                if names != []:
                                    break
...
def get_summary(document):
    ...
    sum_summary = ''
    exact_matches = ['SUMMARY','PROFESSIONAL SUMMARY','EXPERIENCE SUMMARY','PROFILE']
    contains = ['OBJECTIVE']
    for line in document:
...
def get_university(document):
    ...
    univ_terms = ['college','university','institute','academy','school','vishwavidyalaya','polytechnic']
    uni = []
    for line in document:
        line = line[line.find(':')+1:].strip() if line.find(':')!=-1 else line.strip()
        if any(word in line.lower() for word in univ_terms) and len(line)>2 :
            #logger.warning(word)
            if line not in uni:
                univ = next((x.strip().lower() for x in re.split(',|[|]|[-]|[(]|[)]|\sfrom\s|\sin\s',line) if any(word in x.lower() for word in univ_terms)), None)
...
def getLinkedIn(links):
    ...
    for personLinks in links:
        temp = list(filter(lambda x: re.search(
            r'inkedin\.com/in/', x, re.IGNORECASE), personLinks))
        if temp:
...
def extractDataJob(jobExperience,finder,company_tagger):
    ...
    partions = []
    partion_lines = []
    companyWords = ['PRIVATE','LIMITED','LTD', 'CO.', 'CORP','COMPANY', 'UNIVERSITY','PVT','INSTITUTE','SCHOOL''.COM','INC','LLC','IT.','HOLDING','INTERNATIONAL']
    additionalJobWords = ['INTERN','ENGINEER','LEAD','ARCHITECT','MANAGER','DEVELOPER','ANALYST','CONSULTANT','BUSINESS INTELLIGENCE','DIRECTOR','ROLE','SENIOR','TRAINEE','SR.']
    job_labels = ['DESIGNATION','ROLE']
    company_labels = ['CLIENT','ORGANIZATION','PROJECT','TITLE','CUSTOMER','CLIENT DOMAIN','STAKEHOLDERS']
    skills_labels = ['ENVIRONMENT','SKILLS','APPLICATIONS','DOMAIN','INDUSTRY','SERVICE','DESCRIPTION','RESPONSIBILITES','SIZE','PERIOD','DURATION','TECHNOLOGY','TOOLS','ACHIEVEMENTS']
    tech_labels = ['technologies','environment','applications','skills']
    responsiblity_labels = ['responsibilites','accountabilities','description','tasks']
    ...
    if partions != []:
      ...
      for index,line in enumerate(jobExperience):
        ...
        if index<partions[current]+3 and index != partions[current]:
          ...
          job_found = bool((job_check!=None and re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(job_check.lower()),line.lower()) != []) or any(re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(word.lower()),line.lower()) != [] for word in additionalJobWords))
          company_found  = bool((company_check!=None or any(re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(word.lower()),line.lower()) != [] for word in companyWords)) and (len(line.split())<7 or index < partions[current]))#any(word in line.upper() for word in companyWords)
          if ':' in line and line.strip() != line[:line.find(':')+1].strip() and len(line[:line.find(':')].strip())>2:   
            label = True
            job_label = bool(any(word in line[:line.find(':')].upper() for word in job_labels))
            job_found = bool(job_found or job_label)
            company_label = bool(any(word in line[:line.find(':')].upper() for word in company_labels))
            company_found  = bool(company_found or company_label)
            skills_label = bool(any(word in line[:line.find(':')].upper() for word in skills_labels))
          ...
          job_found = bool((job_check!=None and re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(job_check.lower()),line.lower()) != [] ) or any(re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(word.lower()),line.lower()) != [] for word in additionalJobWords))
          company_found  = bool(company_check!=None or any(re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(word.lower()),line.lower()) != [] for word in companyWords))#any(word in partion_lines[current].upper() for word in companyWords)
          if ':' in partion_lines[current] and partion_lines[current].strip() != partion_lines[current][:partion_lines[current].find(':')+1].strip() and len(partion_lines[current][:partion_lines[current].find(':')].strip())>2:   
            label = True
            job_label = bool(any(word in partion_lines[current][:partion_lines[current].find(':')].upper() for word in job_labels))
            job_found = bool(job_found or job_label)
            company_label = bool(any(word in partion_lines[current][:partion_lines[current].find(':')].upper() for word in company_labels))
            company_found  = bool(company_found or company_label)
            skills_label = bool(any(word in partion_lines[current][:partion_lines[current].find(':')].upper() for word in skills_labels))    
          partion_lines[current] = partion_lines[current][partion_lines[current].find(':')+1:].strip() if partion_lines[current].find(':')!=-1 else partion_lines[current].strip() 
        ...
        if index != partions[current] and not ('ROLE'in line.upper() and 'RESPONSIBILITIES'in line.upper()):
          ...
          if company_label:
            ...
            check = re.split(",|;|-|[|]",partion_lines[current].strip())
            if len(check)>0 and (' at ' in check[0]) and (job_check!=None and job_check in check[0]) and (company_check != None and company_check in check[0]):
              ...
              for item in check:
                if (job_check != None and job_check in item) or any(word in item.upper() for word in additionalJobWords):
                  ...
                  company = item.strip()
            elif len(check) >0 and (company_check != None and company_check in check[0]) and not (job_check != None and job_check in check[0]):#any(word in check[0].upper() for word in companyWords)
              company =  check[0]
      ...
      for index,line in enumerate(jobExperience):
        ...
        job_found = bool((job_check!=None and re.findall('([^a-zA-Z]|^){}([^a-zA-Z]|$)'.format(job_check.lower()),line.lower()) != []) or 'INTERN' in line.upper() or 'SR.' in line.upper())
        company_found  = bool(company_check!=None  and len(line.split())<6 )#any(word in line.upper() for word in companyWords)
        ...
        if len(line)>0 and (line[0] != '(' or line[-1] != ')') and len(line.strip().split(' '))<7 and not skills_label and not ('ROLE'in line.upper() and 'RESPONSIBILITIES'in line.upper()):#and not any(word in line.upper() for word in description_words)
          if ' at ' in line:
            ...
            if len(check)>1 and not label:
              for item in check:
                if (any(word in item.upper() for word in additionalJobWords) or (job_check != None and job_check in item)) or  job_label:#any(word in item.upper() for word in companyWords)
                  position = item
                elif ((company_check != None and company_check in item) or company_label) and not company_added:#(any(word in item.upper() for word in companyWords) and not any(word in item.upper() for word in additionalJobWords))
...
def getEduInfo(education):
    ...
    accolades = []
    majors = []
    keywords = ["BACHELOR", "MASTER", "DEGREE", "MS ", "M.S.","B.TECH", "B. TECH","B.E","BACHELORS","M.TECH","M. TECH",'BACHERLORS','DIPLOMA','BTECH','M.C.A','MS:','PHD','B.SC','MBA ','MA -']
    bachelor_keywords = ["BACHELOR","B.TECH","B. TECH","B.E","BACHELORS",'BACHERLORS','BTECH','B.C.A','B.SC'] 
    masters_keywords = ["MASTER", "MS ", "M.S.","M.TECH","M. TECH",'M.C.A','MBA','MS:','MA -']
    PHD_keywords = ['DOCTOR','PH.D','PHD','POSTGRADUATE']
    univ_terms = ['college','university','institute','academy','school','vishwavidyalaya']
            ...
            if len(check)>1:
                for i in check:
                    if any(word in i.upper() for word in keywords):
                        ...
                        if ' in ' in degree or ',' in degree:
                            split_edu = re.split("\sin\s|,|:|\swith\s|\sfrom\s|[(]|[)]",degree)
                        else:
                            split_edu = re.split("\sin\s|\sof\s|,|:|\swith\s|\sfrom\s|[(]|[)]",degree) 

                        ind =  [index for index in range(len(split_edu)) if any(word in ' ' + split_edu[index].upper() + ' ' for word in keywords)]
                        majors.append(split_edu[ind[0]+1] if len(ind)!= 0 and len(split_edu)>1 and ind[0]+1<len(split_edu) and not any(word in line.lower() for word in univ_terms) else '')                                
                ...
                if  len(re.split("\sin\s|\sof\s|,|:|\swith\s|\sfrom\s|[(]|[)]",degree))==1 and index<len(education)-1:
                    ...
                    if start!=0 or end!=len(degree):
                        majors.append(degree[start:end] if not any(word in degree[start:end].lower() for word in univ_terms) else '')
                    else:
                        majors.append(education[index+1] if not any(word in education[index+1].lower() for word in univ_terms) else '')
                else:
                    if ' in ' in degree or ',' in degree:
                        split_edu = re.split("\sin\s|,|:|\swith\s|\sfrom\s|[(]|[)]",degree)
                    else:
                    ...
                    ind =  [index for index in range(len(split_edu)) if any(word in ' ' + split_edu[index].upper() + ' ' for word in keywords)]
                    majors.append(split_edu[ind[0]+1] if len(ind)!= 0 and len(split_edu)>1 and ind[0]+1<len(split_edu) and not any(word in split_edu[ind[0]+1].lower() for word in univ_terms) else '')
